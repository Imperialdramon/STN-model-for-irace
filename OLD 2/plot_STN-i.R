...
# TODO: Agregar parámetros opcionales (--parame=value):
# - infolder: Input folder with STN objects (required)
# - outfolder: Output folder for STN plots (required)
# - bmin: Boolean indicating minimisation or maximisation problem (default TRUE for minimisation)
# - show_regular: Boolean indicating whether to show regular nodes in the plots (default TRUE)
# - size_factor: Factor to scale node and edge sizes in the plots (default 1)
# - asize: Arrow size for plots (default 0.3)
# - ecurv: Curvature of the edges (0 = non, 1 = max) (default 0.3)
# - nsizef: Factor to multiply for node sizes (default 1)
# - ewidthf: Factor to multiply for edge widths (default )
# - mylay: Graph layout as a parameter, as different situations require different layouts (numeric options)
# - palette: Color palette for nodes and edges, e.g., "Set1", "Set2", etc. from RColorBrewer
# - legend: Boolean indicating whether to show legend in the plots
# - legend_position: Position of the legend in the plots (default "topleft")
# - legend_size: Size of the legend text (default 0.7)
...
stn_decorate <- function(N, bmin)  {
  # TODO: Parameter to Eliminate regular nodes, as they are not useful for visualisation
  #N <- delete_vertices(N, V(N)[Quality == "regular"])
  ...
  # Shape of nodes
  V(N)[V(N)$Type == "STANDARD"]$shape = "circle"
  V(N)[V(N)$Type == "START"]$shape = "square"
  V(N)[V(N)$Type == "END"]$shape = "triangle"

  # Color of Nodes
  V(N)[V(N)$Quality == "REGULAR"]$color = regular_ncol
  V(N)[V(N)$Quality == "ELITE"]$color = elite_ncol
  V(N)[V(N)$Quality == "BEST"]$color = best_ncol

  # Size of Nodes Proportional to incoming degree, with at least 1
# This is the number of times a node has been visited, so it is a measure of importance
...
stn_plot <- function(inst)  {
  ...
  # Recomendado
  #lkk <- layout_with_kk(STN)
  #lfr <- layout_with_fr(STN)
